...
class Stories extends ChangeNotifier{
  ...
  final url = Uri.parse("${AppConfig.TestClientUrl}/api/stories/createStory/$userId/");
  var request = http.MultipartRequest('POST', url);

  var file = await http.MultipartFile.fromPath('story_image', storyImage.path);
  ...
  if (response.statusCode == 201) {
    ...
    _currentUser!.hasStory = true;
    notifyListeners();
  } else {
    throw Exception("Erreur lors de la création de la story. Code: ${response.statusCode}");
  }
}
